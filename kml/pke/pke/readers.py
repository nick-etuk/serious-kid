...
class RawTextReader(Reader):
    ...
    def read(self, text, spacy_model=None):
        ...
        if nlp is None:
            ...
            # n.etuk installed_models = [m for m in spacy.util.get_installed_models() if m[:2] == self.language]
            installed_models = ['en_core_web_sm']

            # select first model for the language
            if len(installed_models):
                nlp = spacy.load(installed_models[0], disable=[
                                 'ner', 'textcat', 'parser'])

            # stop execution is no model is available
            else:
                logging.error(
                    'No spacy model for \'{}\' language.'.format(self.language))
                logging.error(
                    'A list of available spacy models is available at https://spacy.io/models.')
                return
